...
import { ActivityID, Activity, renderContentType } from 'niconico/feed';
...
export class FeedView {
    ...
    /** The timestamp of the last visible activity in the viewport. Updated
     * when the viewport is resized or scrolled, but not when an activity
     * is inserted or removed.
     *
     * It is also updated (to null) when the feed is cleared, because that
     * unintentionally triggers the DOM "scroll" event. I can't think of a
     * good way to get rid of that. So consumers need to ignore it if
     * needed. */
    public readonly lastVisibleTimestampChanged: Bacon.EventStream<Date|undefined>;
    ...
    public constructor(ctx = document) {
        ...
        this.lastVisibleTimestampChanged =
            this.feedScrolled
                .throttle(200)
                .map(() => {
                    return this.findLastVisibleTimestamp();
                })
                .skipDuplicates();
    }
    ...
    /** Find the timestamp of the last activity which is fully visible, or
     * null if no activities are shown at all. This method is very
     * frequently called (every time the user scrolls the window) so it
     * needs to be very fast.
     */
    private findLastVisibleTimestamp(): Date|undefined {
        ...
        if (elems.length == 0) {
            ...
            let foundElem: Element|undefined = undefined;
            ...
            if (foundElem) {
                ...
                return new Date((lastElem as HTMLElement).dataset["timestamp"]!);
            }
            else {
